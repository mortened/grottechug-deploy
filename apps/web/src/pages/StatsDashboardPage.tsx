...
export function StatsDashboardPage() {
    ...
    const base =
      data?.timeSeries.map((x) => {
        ...
        const wetCount = violations.filter(
          (v) =>
            v.dateISO.slice(0, 10) === x.dateISO.slice(0, 10) &&
  ...
  const overallWetRate = useMemo(() => {
    ...
    const wetCount = violations.filter((v) =>
      ["W", "VW", "MM", "P", "T"].includes(v.ruleCode),
    ).length;
    return (wetCount / data.overview.attempts) * 100;
  }, [violations, data]);

  const chugsPerSession = data?.overview.sessions
    ? data.overview.attempts / data.overview.sessions
    : 0;

  const validParticipants = participantStats.filter(
    (p) => p.attempts > 0 && p.avg !== null,
  );
  ...
  const slowestPerson =
    qualifiedForAwards.length > 0
      ? qualifiedForAwards.reduce((prev, current) =>
          (current.avg || 0) > (prev.avg || 0) ? current : prev,
        )
      : null;
  ...
  const fastestPerson =
    qualifiedForAwards.length > 0
      ? qualifiedForAwards.reduce((prev, current) =>
          (current.avg || Infinity) < (prev.avg || Infinity) ? current : prev,
        )
      : null;
  ...
  const scatterData = validParticipants
    .filter(
      (p) =>
        p.isRegular || (showScatterGuests && !p.isRegular && p.attempts >= 3),
    )
    .map((p) => ({
      name: p.name,
      attempts: p.attempts,
  ...
  const lowestWetRateData = useMemo(() => {
    return validParticipants
      ...
      .map((p) => {
        const wetCount = violations.filter(
          (v) =>
            v.participantId === p.participantId &&
            ["W", "VW", "MM", "P", "T"].includes(v.ruleCode),
        ).length;
  ...
  const improvementData = useMemo(() => {
    ...
    const sortedCols = [...tableData.columns].sort(
      (a, b) => new Date(a.dateISO).getTime() - new Date(b.dateISO).getTime(),
    );

    const improvements = tableData.rows
      .map((r) => {
        const times: number[] = [];
        ...
        const lastTwoAvg =
          (times[times.length - 1] + times[times.length - 2]) / 2;
          ...
          {data.fastestPerson && (
            ...
            >
              ...
              <span style={{ textAlign: "right" }}>
                <strong style={{ color: getColor(data.fastestPerson) }}>
                  {data.fastestPerson}
                </strong>{" "}
                ({data.fastestTime?.toFixed(2)}s)
              </span>
            </div>
          )}
          {data.slowestPerson && (
            ...
            >
              ...
              <span style={{ textAlign: "right" }}>
                <strong style={{ color: getColor(data.slowestPerson) }}>
                  {data.slowestPerson}
                </strong>{" "}
                ({data.slowestTime?.toFixed(2)}s)
              </span>
            </div>
          )}
        </div>
    ...
    if (active && payload && payload.length) {
      return (
        ...
        >
          ...
          <div style={{ color: "#ffffff", fontWeight: 600 }}>
            Straffeprosent: {payload[0].value.toFixed(1)}%
          </div>
        </div>
      );
    }
    ...
    if (active && payload && payload.length) {
      return (
        ...
        >
          <div style={{ marginBottom: 6, color: "var(--muted)" }}>{label}</div>
          <div style={{ color: "#ffffff", fontWeight: 600 }}>
            Wet-rate: {payload[0].value.toFixed(1)}%
          </div>
        </div>
      );
    }
      ...
      return (
        ...
        >
          ...
          <div style={{ color: "#ffffff", fontWeight: 600, marginBottom: 8 }}>
            Forbedring:{" "}
            <span
              style={{ color: data.improvementPct > 0 ? "#10b981" : "#ef4444" }}
            >
              {data.improvementPct > 0 ? "+" : ""}
              {data.improvementPct.toFixed(1)}%
            </span>
          </div>
          <div
            style={{
          ...
          >
            <span>Snitt 2 første:</span>{" "}
            <span>{data.firstAvg.toFixed(2)}s</span>
          </div>
          <div
            style={{
              color: "var(--muted)",
        ...
        <>
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontWeight: 900,
                  color:
                    overallWetRate > 20 ? "var(--danger)" : "var(--accent2)",
                  lineHeight: 1,
                }}
              >
          ...
          <div className="row" style={{ marginTop: 14, flexWrap: "wrap" }}>
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Basert på MM, W, VW, P og T-kryss.
              </div>
              <div style={{ width: "100%", height: 300 }}>
                <ResponsiveContainer>
                  <AreaChart
                  ...
                  >
                    ...
                    <Tooltip
                      ...
                      contentStyle={{
                        backgroundColor: "rgba(18,26,51,0.95)",
            ...
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Fordeling av alle registrerte anmerkninger.
              </div>
              <div style={{ width: "100%", height: 300 }}>
                <ResponsiveContainer>
                  <BarChart
                  ...
                  >
                    ...
                    <Tooltip
                      content={<CustomNoteTypesTooltip />}
          ...
          <div className="row" style={{ marginTop: 14, flexWrap: "wrap" }}>
            ...
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Snitt av to første chugs vs to siste (krever ≥ 4 chugs).
              </div>
              <div style={{ width: "100%", height: 280 }}>
                {improvementData.length > 0 ? (
                  <ResponsiveContainer>
                    ...
                    >
                      ...
                      />
                      <Tooltip
                        content={<CustomImprovementTooltip />}
                        cursor={{ fill: "rgba(255,255,255,0.05)" }}
                      />
            ...
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Totalt antall chugs registrert hver dato.
              </div>
              <div style={{ width: "100%", height: 280 }}>
                <ResponsiveContainer>
                  <BarChart
          ...
          <div
            className="row"
            style={{ marginTop: 14, flexWrap: "wrap", marginBottom: 40 }}
          >
            {/* Lavest Wet-Rate */}
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Krever minst 5 registrerte chugs for å kvalifisere.
              </div>
              <div style={{ width: "100%", height: 280 }}>
                {lowestWetRateData.length > 0 ? (
                  <ResponsiveContainer>
                    <BarChart
                      data={lowestWetRateData}
                      margin={{ top: 10, right: 10, bottom: 0, left: -20 }}
                    >
                      <CartesianGrid
                        strokeDasharray="3 3"
            ...
            <div className="col card" style={{ flex: "1 1 400px" }}>
              ...
              >
                Andel runder som får en anmerkning (min. 3 forsøk).
              </div>
              <div style={{ width: "100%", height: 280 }}>
                {hasParticipantStats ? (
                  <ResponsiveContainer>
                    ...
                    >
                      ...
                      />
                      <Tooltip
                        content={<CustomPunishmentTooltip />}
                        cursor={{ fill: "rgba(255,255,255,0.05)" }}
                      />
